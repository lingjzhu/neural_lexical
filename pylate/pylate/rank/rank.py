...
from ..scores import colbert_scores, colbert_scores_masked_mean
...
def rerank_meanpool(
    ...
    documents_embeddings,
    queries_masks=None,
    documents_masks=None,
):
    """
    Rerank documents using masked mean-pooled ColBERT scores.

    Args:
        documents_ids: list[list[str]] or list[str]
        queries_embeddings: (B_q, Tq, K)
        documents_embeddings: (B_d, Td, K)
        queries_masks: (B_q, Tq)
        documents_masks: (B_d, Td)
    Returns:
        reranked_documents: list[list[str]]
    """
    # (B_q, B_d)
    scores = colbert_scores_masked_mean(
        queries_embeddings=queries_embeddings,
        documents_embeddings=documents_embeddings,
        queries_mask=queries_masks,
        documents_mask=documents_masks,
    )

    # sort documents for each query
    ranked_indices = torch.argsort(scores, dim=-1, descending=True)

    reranked_documents = []
    for q_idx in range(scores.size(0)):
        reranked_documents.append(
            [documents_ids[d_idx] for d_idx in ranked_indices[q_idx].tolist()]
        )
