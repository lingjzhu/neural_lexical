...
def use_kernelized_func(*args, **kwargs):
    def decorator(cls):
        return cls
    return decorator
def create_bidirectional_mask(config, input_embeds, attention_mask):
    if attention_mask is None:
        return None
    mask = attention_mask[:, None, None, :]
    mask = mask.to(dtype=input_embeds.dtype)
    return (1.0 - mask) * torch.finfo(input_embeds.dtype).min

def create_bidirectional_sliding_window_mask(config, input_embeds, attention_mask):
    # Fallback to normal bidirectional mask without sliding window behavior for eager
    # This is a simplification since local 4.57 doesn't have the explicit mask function.
    return create_bidirectional_mask(config, input_embeds, attention_mask)
...
def maybe_autocast(device_type=None, enabled=True, **kwargs):
