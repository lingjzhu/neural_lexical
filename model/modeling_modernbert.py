...
class ModernBertForEmbeddingsOpt(ModernBertPreTrainedModel):
    _tied_weights_keys = ["decoder.weight"]
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config._attn_implementation == "flash_attention_2":
            ...
            if hs.dim() == 3 and hs.shape[0] == 1:
                hs = hs.squeeze(0)
            last_hidden_state = _pad_modernbert_output(inputs=hs, indices=indices, batch=batch_size, seqlen=seq_len)


        # mean pooling the last hidden states
        last_hidden_state = (last_hidden_state * attention_mask.unsqueeze(-1)).sum(dim=1) / attention_mask.sum(
                dim=1, keepdim=True
            )
...
class ModernBertForEmbeddings(ModernBertPreTrainedModel):
    _tied_weights_keys = ["decoder.weight"]
    ...
    def forward(
        ...
        **kwargs,
        ...
        # mean pooling the last hidden states
        #last_hidden_state = self.head(last_hidden_state * attention_mask.unsqueeze(-1))

        logits = last_hidden_state # place holder only #self.decoder(last_hidden_state)
...
class ModernBertForEmbeddingsFusedMaxpool(ModernBertPreTrainedModel):
    _tied_weights_keys = ["decoder.weight"]
    ...
    def forward(
        ...
        **kwargs,
        ...
        logits = splade_max_pool(last_hidden_state, self.decoder.weight)
...
class ModernBertForEmbeddingsFusedMeanpool(ModernBertPreTrainedModel):
    _tied_weights_keys = ["decoder.weight"]
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not self.fused:
            logits = self.decoder(last_hidden_state)  # [B, T, V]
            logits = torch.sum(torch.relu(logits),dim=1) # [B, V]
        else:
            logits = feature_sum_pool(last_hidden_state, self.decoder.weight)
...
class ModernBertForSparseColbert(ModernBertPreTrainedModel):
    _tied_weights_keys = ["decoder.weight"]
    ...
    def __init__(self, config: ModernBertConfig):
        ...
        #self.sparse_prediction = self.config.sparse_prediction
        #self.sparse_pred_ignore_index = self.config.sparse_pred_ignore_indexs
        self.k = 64
        self.triton = True
        ...
        **kwargs,
        ...
        if self.triton:
            ...
            if self.config._attn_implementation == "flash_attention_2":
                ...
                # Hidden states padding
                if  last_hidden_state.dim() == 3 and last_hidden_state.shape[0] == 1:
            ...
            with torch.autocast(device_type=last_hidden_state.device.type,dtype=last_hidden_state.dtype):
                top_vals, top_inds = sparse_colbert_topk(last_hidden_state, self.decoder.weight, self.k)
