...
@triton.jit
def _pairwise_maxsim_fwd_kernel(
    Q_Vals_ptr, Q_Inds_ptr,
    D_Vals_ptr, D_Inds_ptr,
    Scores_ptr, Argmax_ptr,
    B1, B2, Tq, Td, K: tl.constexpr,
    # Strides
    stride_q_b, stride_q_t, stride_q_k,
    stride_d_b, stride_d_t, stride_d_k,
    stride_s_b1, stride_s_b2,
    stride_a_b1, stride_a_b2, stride_a_t,
    BLOCK_TD: tl.constexpr
):
    # Grid: (Tq, B1, B2)
    pid_tq = tl.program_id(0)
    pid_b1 = tl.program_id(1) # Index for Query Batch
    pid_b2 = tl.program_id(2) # Index for Doc Batch

    # --- Load Query (from B1) ---
    q_ptr_base_v = Q_Vals_ptr + pid_b1 * stride_q_b + pid_tq * stride_q_t
    q_ptr_base_i = Q_Inds_ptr + pid_b1 * stride_q_b + pid_tq * stride_q_t
    
    offsets_k = tl.arange(0, K)
    #q_inds = tl.load(q_ptr_base_i + offsets_k * stride_q_k)
    #q_vals = tl.load(q_ptr_base_v + offsets_k * stride_q_k)

    # --- Setup Doc Pointers (from B2) ---
    d_ptr_base_v = D_Vals_ptr + pid_b2 * stride_d_b
    d_ptr_base_i = D_Inds_ptr + pid_b2 * stride_d_b

    # Track Max
    max_sim = 0.0
    best_idx = -1 # Default: No match found

    # --- Loop over Doc Tokens ---
    for start_td in range(0, Td, BLOCK_TD):
        offsets_td = start_td + tl.arange(0, BLOCK_TD)
        mask_td = offsets_td < Td
        
        # Load D Chunk
        # Broadcast D pointers: [BLOCK_TD, 1] + [1, K]
        chunk_ptr_v = d_ptr_base_v + offsets_td[:, None] * stride_d_t + offsets_k[None, :] * stride_d_k
        chunk_ptr_i = d_ptr_base_i + offsets_td[:, None] * stride_d_t + offsets_k[None, :] * stride_d_k
        
        mask_load = mask_td[:, None]
        d_inds_chunk = tl.load(chunk_ptr_i, mask=mask_load)
        d_vals_chunk = tl.load(chunk_ptr_v, mask=mask_load)

        # Dot Product (Q broadcasted vs D chunk)
        dot_products = tl.zeros([BLOCK_TD], dtype=tl.float32)
        for k_q in range(K):
            cur_q_ind = tl.load(q_ptr_base_i + k_q * stride_q_k)
            cur_q_val = tl.load(q_ptr_base_v + k_q * stride_q_k)
            
            match = (d_inds_chunk == cur_q_ind)
            prod = d_vals_chunk * cur_q_val
            dot_products += tl.sum(tl.where(match, prod, 0.0), axis=1)

        dot_products = tl.where(mask_td, dot_products, float("-inf"))
        
        block_max_val, block_max_idx = tl.max(dot_products, axis=0, return_indices=True)
        
        if block_max_val > max_sim:
            max_sim = block_max_val
            best_idx = start_td + block_max_idx

    # --- Store Results ---
    # Score: Atomic Add to (B1, B2)
    score_offset = pid_b1 * stride_s_b1 + pid_b2 * stride_s_b2
    tl.atomic_add(Scores_ptr + score_offset, max_sim)
    
    # Argmax: Direct Store to (B1, B2, Tq)
    # This tensor is unique per thread, no atomic needed
    argmax_offset = pid_b1 * stride_a_b1 + pid_b2 * stride_a_b2 + pid_tq * stride_a_t
    tl.store(Argmax_ptr + argmax_offset, best_idx)


# --- 2. Pairwise Backward Kernel ---
@triton.jit
def _pairwise_maxsim_bwd_kernel(
    ...
    # Grid: (Tq, B1, B2)
    # We are calculating the contribution of Q[b1] vs D[b2]
    pid_tq = tl.program_id(0)
    ...
    # Forward pass clamp check
    if best_idx == -1:
        return

    # 2. Load Gradient Scalar
    grad_val = tl.load(Grad_Score_ptr + pid_b1 * stride_gs_b1 + pid_b2 * stride_gs_b2)

    # 3. Load Q[b1, tq]
    q_ptr_base = pid_b1 * stride_q_b + pid_tq * stride_q_t
    offsets_k = tl.arange(0, K)
    q_inds = tl.load(Q_Inds_ptr + q_ptr_base + offsets_k * stride_q_k)
    q_vals = tl.load(Q_Vals_ptr + q_ptr_base + offsets_k * stride_q_k)

    # 4. Load D[b2, best_idx]
    d_ptr_base = pid_b2 * stride_d_b + best_idx * stride_d_t
    d_inds = tl.load(D_Inds_ptr + d_ptr_base + offsets_k * stride_d_k)
    d_vals = tl.load(D_Vals_ptr + d_ptr_base + offsets_k * stride_d_k)

    # 5. Compute Gradients (Vectorized)
    # Matches [K, K]
    matches = (q_inds[:, None] == d_inds[None, :])
    
    # dQ contribution (sum over D cols)
    dq_local = tl.sum(tl.where(matches, d_vals[None, :], 0.0), axis=1) * grad_val
    
    # dD contribution (sum over Q rows)
    dd_local = tl.sum(tl.where(matches, q_vals[:, None], 0.0), axis=0) * grad_val
    
    # 6. Accumulate Gradients
    # CRITICAL: Both must be atomic_add because:
    #   - Q[b1] is used by multiple D's (across B2 dim)
    #   - D[b2] is used by multiple Q's (across B1 dim)
    
    # dQ ptr
    dq_ptr = DQ_Vals_ptr + pid_b1 * stride_dq_b + pid_tq * stride_dq_t
    tl.atomic_add(dq_ptr + offsets_k * stride_dq_k, dq_local)
    
    # dD ptr
    dd_ptr = DD_Vals_ptr + pid_b2 * stride_dd_b + best_idx * stride_dd_t
    tl.atomic_add(dd_ptr + offsets_k * stride_dd_k, dd_local)


# --- 3. Wrapper ---
class PairwiseSparseMaxSim(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q_vals, q_inds, d_vals, d_inds):
        q_vals, q_inds = q_vals.contiguous(), q_inds.contiguous()
        d_vals, d_inds = d_vals.contiguous(), d_inds.contiguous()
        
        B1, Tq, K = q_vals.shape
        B2, Td, _ = d_vals.shape
        
        # Output Scores: (B1, B2)
        scores = torch.zeros((B1, B2), device=q_vals.device, dtype=torch.float32)
        # Argmax Indices: (B1, B2, Tq) - This stores the winner for every pair
        argmax_inds = torch.empty((B1, B2, Tq), device=q_vals.device, dtype=torch.int32)
        
        grid = (Tq, B1, B2)
        BLOCK_TD = 32
        
        _pairwise_maxsim_fwd_kernel[grid](
            q_vals, q_inds,
            d_vals, d_inds,
            scores, argmax_inds,
            B1, B2, Tq, Td, K=K,
            stride_q_b=q_vals.stride(0), stride_q_t=q_vals.stride(1), stride_q_k=q_vals.stride(2),
            stride_d_b=d_vals.stride(0), stride_d_t=d_vals.stride(1), stride_d_k=d_vals.stride(2),
            stride_s_b1=scores.stride(0), stride_s_b2=scores.stride(1),
            stride_a_b1=argmax_inds.stride(0), stride_a_b2=argmax_inds.stride(1), stride_a_t=argmax_inds.stride(2),
            BLOCK_TD=BLOCK_TD
        )
        
        ctx.save_for_backward(q_vals, q_inds, d_vals, d_inds, argmax_inds)
        return scores

    @staticmethod
    def backward(ctx, grad_output):
        # grad_output: (B1, B2)
        q_vals, q_inds, d_vals, d_inds, argmax_inds = ctx.saved_tensors
        B1, Tq, K = q_vals.shape
        B2, Td, _ = d_vals.shape
        
        dq_vals = torch.zeros_like(q_vals)
        dd_vals = torch.zeros_like(d_vals)
        
        grid = (Tq, B1, B2)
        
        _pairwise_maxsim_bwd_kernel[grid](
            grad_output, argmax_inds,
            q_vals, q_inds,
            d_vals, d_inds,
            dq_vals, dd_vals,
            B1, B2, Tq, Td, K=K,
            stride_gs_b1=grad_output.stride(0), stride_gs_b2=grad_output.stride(1),
            stride_a_b1=argmax_inds.stride(0), stride_a_b2=argmax_inds.stride(1), stride_a_t=argmax_inds.stride(2),
            stride_q_b=q_vals.stride(0), stride_q_t=q_vals.stride(1), stride_q_k=q_vals.stride(2),
            stride_d_b=d_vals.stride(0), stride_d_t=d_vals.stride(1), stride_d_k=d_vals.stride(2),
            stride_dq_b=dq_vals.stride(0), stride_dq_t=dq_vals.stride(1), stride_dq_k=dq_vals.stride(2),
            stride_dd_b=dd_vals.stride(0), stride_dd_t=dd_vals.stride(1), stride_dd_k=dd_vals.stride(2)
        )
        
        return dq_vals, None, dd_vals, None
...
def sparse_maxsim_pairwise(q, d, q_mask=None, d_mask=None, normalize=True):
    ...
    for i in range(B):
        # kernel call with batch size = 1
        s = PairwiseMaxSimFp8(#_sparse_maxsim(#PairwiseSparseMaxSim.apply(
...
def torch_chunked_topk_sum_sim(q_vals, q_inds, d_vals, d_inds, top_n=3, chunk_size=10):
    """
    Computes MaxSim but sums the Top-N best matching tokens instead of just the Max.
    
    Args:
        top_n: Number of top document tokens to sum for each query token.
        chunk_size: Batch size for processing (prevents OOM).
    """
    B1, Tq, K = q_vals.shape
    B2, Td, _ = d_vals.shape
    
    # 1. Determine Vocab & Materialize Query
    V = max(q_inds.max().item(), d_inds.max().item()) + 1
    
    Q_dense = torch.zeros((B1, Tq, V), device=q_vals.device, dtype=q_vals.dtype)
    Q_dense.scatter_(2, q_inds.long(), q_vals)
    
    final_scores = []
    
    # Safety: Cannot take top-N if document is shorter than N
    # We clamp N to be at most the document length
    effective_n = min(top_n, Td)
    
    # 2. Process Chunks
    for i in range(0, B2, chunk_size):
        end = min(i + chunk_size, B2)
        batch_len = end - i
        
        # Slice Sparse Inputs
        d_v_chunk = d_vals[i:end]
        d_i_chunk = d_inds[i:end]
        
        # Materialize Dense Doc Chunk
        D_dense = torch.zeros((batch_len, Td, V), device=d_vals.device, dtype=d_vals.dtype)
        D_dense.scatter_(2, d_i_chunk.long(), d_v_chunk)
        
        # Matrix Multiply (Tensor Cores)
        # Result: [Batch, Tq, Td]
        sim_matrix = torch.bmm(Q_dense.expand(batch_len, -1, -1), D_dense.transpose(1, 2))
        
        # --- NEW LOGIC: Top-N Sum ---
        
        # 1. Find Top-N values along the Document Dimension (dim=2)
        # shape: [Batch, Tq, N]
        top_vals = torch.topk(sim_matrix, k=effective_n, dim=2).values
        
        # 2. Sum these N values to get score per query token
        # shape: [Batch, Tq]
        token_scores = top_vals.sum(dim=2)
        
        # 3. Sum over Query Tokens to get final Document Score
        # shape: [Batch]
        scores = token_scores.sum(dim=1)
        
        final_scores.append(scores)
        
    return torch.cat(final_scores).unsqueeze(0)