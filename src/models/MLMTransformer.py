...
class MLMTransformer(InputModule):
    ...
    def _load_model(
        ...
        **model_args,
        ...
        if backend == "modernbert":
            self.auto_model = ModernBertForEmbeddings.from_pretrained(model_name_or_path, config=config, **model_args)

        elif backend == "modernbert_fused_max":
            self.auto_model = ModernBertForEmbeddingsFusedMaxpool.from_pretrained(model_name_or_path, config=config, **model_args)

        elif backend == "modernbert_fused_mean":
            self.auto_model = ModernBertForEmbeddingsFusedMeanpool.from_pretrained(model_name_or_path, config=config, **model_args)

        elif backend == "modernbert_sparse_colbert":
            self.auto_model = ModernBertForSparseColbert.from_pretrained(model_name_or_path, config=config, **model_args)

        elif backend == "qwen3":
            from liger_kernel.transformers import apply_liger_kernel_to_qwen3
            apply_liger_kernel_to_qwen3()
            self.auto_model = Qwen3ForEmbedding.from_pretrained(model_name_or_path, config=config, **model_args)

        elif backend == "qwen3_fused_max":
            from liger_kernel.transformers import apply_liger_kernel_to_qwen3
            apply_liger_kernel_to_qwen3()
            self.auto_model = Qwen3ForEmbeddingFusedMaxpool.from_pretrained(model_name_or_path, config=config, **model_args)
        
        elif backend == "qwen3_fused_mean":
            from liger_kernel.transformers import apply_liger_kernel_to_qwen3
            apply_liger_kernel_to_qwen3()
            self.auto_model = Qwen3ForEmbeddingFusedMeanpool.from_pretrained(model_name_or_path, config=config, **model_args)

        elif backend == "qwen3_sparse_colbert":
            from liger_kernel.transformers import apply_liger_kernel_to_qwen3
            apply_liger_kernel_to_qwen3()
    ...
    def forward(self, features: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        try:
            if self.backend != "t5gemma":
                features["sparse_embeddings"] = outputs.logits
                features["dense_embeddings"] = outputs.hidden_states
            else:
                features["sparse_embeddings"] = outputs.logits
                features["dense_embeddings"] = outputs.decoder_hidden_states
