...
from src.kernels.fused_maxsim import sparse_maxsim_pairwise, sparse_maxsim
logger = logging.getLogger(__name__)


class ClusteredColbertEncoder(SentenceTransformer):
    """
    ...
    def __init__(
        ...
        embedding_type: Literal["dense", "sparse", "mixed", "colbert"] = "mixed",
        backend: Literal["torch", "onnx", "openvino"] = "torch",
    ) -> None:
        
        object.__setattr__(self, "embedding_type", embedding_type)
    ...
    def encode(
        ...
        **tokenize_kwargs,
        ...
        """
        Encode sentences using ColBERT sparse embeddings.

        Returns
        -------
        If return_padded is False:
            list of tuples:
                (vals, inds)
                where vals, inds have shape (T, K)

        If return_padded is True:
            vals_padded: (B, T_max, K)
            inds_padded: (B, T_max, K)
        """
        self.eval()
        ...
        # ---- sort by length (efficiency) ----
        length_sorted_idx = np.argsort([-self._text_length(s) for s in sentences])
        sentences_sorted = [sentences[i] for i in length_sorted_idx]
        ...
        for start in trange(
            ...
            batch_sentences = sentences_sorted[start : start + batch_size]

            features = self.tokenize(batch_sentences, **tokenize_kwargs)
            ...
            with torch.inference_mode():
                ...
                with autocast_ctx:
                    out_embeds = self(features)["sparse_embeddings"]
                    if isinstance(out_embeds, tuple) or (isinstance(out_embeds, list) and len(out_embeds) == 2):
                        vals, inds = out_embeds
                    else:
                        vals = out_embeds
                        inds = None
            ...
            # ---- split batch → per-sample ----
            for i in range(vals.size(0)):
                v = vals[i]    # (T_i, K)
                ind = inds[i] if inds is not None else None  # (T_i, K)
        ...
        inv_idx = np.argsort(length_sorted_idx)
        all_vals = [all_vals[i] for i in inv_idx]
        all_inds = [all_inds[i] for i in inv_idx]
        ...
        # ---- padded output ----
        if return_tensors:
            vals_padded = torch.nn.utils.rnn.pad_sequence(
                all_vals, batch_first=True, padding_value=0.0
            )  # (B, T_max, K)

            if all_inds[0] is not None:
                inds_padded = torch.nn.utils.rnn.pad_sequence(
                    all_inds, batch_first=True, padding_value=0.0
                )  # (B, T_max, K)
            else:
                inds_padded = None

            return vals_padded, inds_padded

        # ---- default: list[(vals, inds)] ----
        return list(zip(all_vals, all_inds))
    ...
    @property
    def similarity_fn_name(self) -> Literal["cosine", "dot", "euclidean", "manhattan", "maxsim"]:
        ...
        if self.embedding_type == "colbert":
            self._similarity_fn_name = "colbert"
        elif self._similarity_fn_name is None:
    ...
    def similarity_fn_name(
        self,
        value: Literal["cosine", "dot", "euclidean", "manhattan", "maxsim"] | SimilarityFunction,
    ) -> None:
        ...
        if self.embedding_type == "colbert":
            self._similarity = sparse_maxsim
            self._similarity_pairwise = sparse_maxsim_pairwise
    ...
    def save(
        ...
        # Attempt to save the clustered keys natively into the folder
        try:
            if hasattr(self[0], "cluster_ids"):
                import torch
                os.makedirs(path, exist_ok=True)
                torch.save(self[0].cluster_ids.cpu(), os.path.join(path, "cluster_ids.pt"))
        except Exception as e:
            print(f"ClusteredColbertEncoder failed to natively save cluster_ids.pt: {e}")
        return res
