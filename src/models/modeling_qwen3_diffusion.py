"""
Bidirectional Qwen3 model for diffusion language modeling (A2D / MDLM).

This is adapted from the dllm library (dllm/pipelines/a2d/models/qwen3/modeling_qwen3.py).
The key change from standard Qwen3 is that the causal mask is replaced with a 
bidirectional (padding-only) attention mask, making the model suitable for masked/diffusion
...
class A2DQwen3Model(transformers.Qwen3Model):
    """Qwen3Model with bidirectional (non-causal) attention."""
...
class A2DQwen3LMHeadModel(transformers.Qwen3ForCausalLM):
    """Qwen3 LM head model with bidirectional A2DQwen3Model backbone."""
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return CausalLMOutputWithPast(
            loss=loss,
            logits=hidden_states,  # Return hidden_states as logits for downstream (consistent with Qwen3ForEmbedding)
            past_key_values=outputs.past_key_values,
        )


# Register with transformers AutoModel registry
